...
////////////////////////////////////////////////////////////////////////////////////////
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    // worried about behavior of less when Node* is nullptr
...
/*    // Base case
    if (empty_impl(node)) {
      node = new Node(item, nullptr, nullptr);
      return node;
    } else if (less(item, node->datum) && empty_impl(node->left)) {
      node->left = new Node(item, nullptr, nullptr);
      return node->left;
    } else if (!less(item, node->datum) && empty_impl(node->right)) {
      node->right = new Node(item, nullptr, nullptr);
      return node->right;
    }
    if (less(item, node->datum) && empty_impl(node->left)) {
      Node new_node(item, nullptr, nullptr); //is this correct?
      node->left = new_node;
    }
    
    if (less(item, node->datum) && empty_impl(node->right)) {
      Node new_node(item, nullptr, nullptr); //correct?
      node->right = new_node;
    }
...
*/
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    if (empty_impl(node) || empty_impl(node->left) || empty_impl(node->right)) {
      return true;
    } 
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if (node == nullptr) {
      ...
      Node *goLeft = min_greater_than_impl(node->left, val, less);
     if(goLeft == nullptr) {
      return node;
    }
    else{
