...
private:
  ...
  class PairComp {
    public:
    Key_compare key;
    bool operator()(const Pair_type &first, const Pair_type &second) {
      return key(first.first, second.first);
    }
  };
...
public:
  ...
  Value_type& operator[](const Key_type& k){
    ...
    return tree.find({k, Value_type()})->second;
  }
